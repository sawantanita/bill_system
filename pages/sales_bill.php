...
<?php
$TABLENAME = "sale_bill";                          // define table name
$CHILDTABLENAME = "";  					            // child table name if any
$PKEY = "id";						           // primary key

$FIELDLIST = "id,prepared_date,purchaser_no,customer_no,product_code,product_name,qty,rate,basic_tot,gst_rate,gst_val,gross_total";		          // table fields 
...
// ****************retriving value from query strin******************** 
	...
	if($_GET[purchaser_no]!="")
		$purchaser_no=$_GET[purchaser_no];
?>
</script>
<html>
<head>
...
{
	winSel = winOpen("sale_bill_list.php","oWin","500","200")
}
function newRecord()
{
...
function saveRecord()
{
	...
	{	
		document.forms[0].action = "sale_bill_store.php"
...
{
	var oTextbox = new AutoSuggestControl(document.getElementById("product_name"), new SuggestionProvider(),"item_quantity","product_name","");  
	//var oTextbox = new AutoSuggestControl(document.getElementById("product_name"), new SuggestionProvider(),"item_quantity","product_name"," ");  
}
...
//alert("okok")
	...
	http.open("GET", 'find_val_exists2.php?fieldvar='+val+'&fieldval='+document.getElementById(val).value+'&purchaser_no='+document.getElementById('non$purchaser_no').value+'&customer_no='+document.getElementById('non$customer_no').value, true);
	http.onreadystatechange = function() {
		...
		else
		{
			...
			document.getElementById('non$stkqty').value=tt[3]
			document.getElementById('purchaser_no').value=tt[4]
			document.getElementById('customer_no').value=tt[5]
		}
		}
	}
	http.send(null);
...
{
	//alert(document.getElementById("qty").value+"  "+document.getElementById('non$stkqty').value)
	if(parseFloat(document.getElementById("qty").value)>parseFloat(document.getElementById('non$stkqty').value) && parseFloat(document.getElementById("qty").value)>0)
	{
		alert("Do Not Enter Qty More Than Stock Qty")
...
function gotochk()
{
	//document.location="sales_bill.php?purchaser_no="+document.forms[0].purchaser_no.value
}
...
function autocust_nm()
{

	var oTextbox = new AutoSuggestControl(document.getElementById("non$customer_no"), new SuggestionProvider(),"customer_master","cust_name","cust_type ='Customer'");  
	//gotochkavail('product_name');
}
</script>
      ...
      <td align="center" valign="top"> <table width="100%" border="0" cellpadding="0">
          <tr> 
            <td height="107"> <table width="500" border="0" align="center">
                <tr class="tablehead"> 
                  <td height="25" colspan="3" align="center"><strong>SALE BILL</strong> </td>
                </tr>
                <tr height="8"> 
            ...
            <input type="hidden"id="prepared_date" name="prepared_date" value="<?=date("Y/m/d")?>">
				...
				<tr> 
                  ...
                  <td><div align="center">&nbsp;:&nbsp;</div></td>
                 <td><!--select id="purchaser_no" name="purchaser_no" onChange="gotochk()">
				...
				<tr> 
                  <td>Customer Name</td>
                  <td><div align="center">&nbsp;:&nbsp;</div></td>
                  <td><!--select id="customer_no" name="customer_no">
				  <option value="">-Select-</option>
				  <?  tbl_list_data ("customer_master","cust_name","id"," where cust_type ='Customer'") ?>
				  </select-->
				   <input type="text"  id="non$customer_no" name="non$customer_no" value="<?=$customer_no?>" onKeyPress="autocust_nm('');"   validate="isNotNull" disName="purchaser_no" > 
				...
				<tr> 
                  ...
                  <br>Stock <input type="text"  id="non$stkqty" name="non$stkqty" value="" style="width:100">
				  </td>
                </tr>
				<tr> 
                  <td>Qty</td>
                  <td><div align="center">&nbsp;:&nbsp;</div></td>
                  <td><input type="text"  id="qty" name="qty" value="<?=$qty?>" onBlur="setamt_toall()" onFocus="gotochkavail('product_name')"  validate="isNotNull" disName="qty" > 
                  </td>
                </tr>
				<tr> 
                  <td>rate</td>
...
// **************** END FUNCTION CALLS used for button disable and enable ******************** '
	...
	if ($selValue == "")
	{
		echo("disableForm() \n");
		echo("newRecord() \n");
		echo("listValue(document.forms[0].purchaser_no, '" . $purchaser_no . "') \n" );
