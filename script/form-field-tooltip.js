...
function DHTMLgoodies_formTooltip()
{
	...
	var delcoockie;  // this is the variable to display or not to display "Don't Show this message again"
	// Reference to form field which tooltip is currently showing for
...
DHTMLgoodies_formTooltip.prototype = {
	...
	__displayTooltip : function()
	{
		...
		if(DHTMLgoodies_globalTooltipObj.isMSIE){
			...
			}else if(DHTMLgoodies_globalTooltipObj.tooltipPosition == 'right'){
					DHTMLgoodies_globalTooltipObj.tooltipIframeObj.style.height = (DHTMLgoodies_globalTooltipObj.tooltipDiv.clientHeight);
			}else if(DHTMLgoodies_globalTooltipObj.tooltipPosition == 'top'){
					DHTMLgoodies_globalTooltipObj.tooltipIframeObj.style.height =10;
			}else if(DHTMLgoodies_globalTooltipObj.tooltipPosition == 'topleft'){
					DHTMLgoodies_globalTooltipObj.tooltipIframeObj.style.height =10;
			}
		}
	...
	__createTooltip : function()
	{
		...
		if(this.displayArrow){
			...
			else if(this.tooltipPosition=='topleft')
			{
				
				topDiv.style.marginTop = '48px';
				topDiv.style.marginLeft = '130px';
				var arrowDiv = document.createElement('IMG');
				arrowDiv.src = this.imagePath + this.arrowImageFileRight + '?rand='+ Math.random();	
				arrowDiv.style.display='block';
				topDiv.appendChild(arrowDiv);					
	...
	{	
		...
		if(this.tooltipPosition=='below'){
			...
			this.tooltipDiv.style.left =(this.getLeftPos(inputObj) + inputObj.offsetWidth + offset-43)+  'px';
			this.tooltipDiv.style.top = this.getTopPos(inputObj)-70 + 'px';
		}else if(this.tooltipPosition=='topleft'){
			this.tooltipDiv.style.left =(this.getLeftPos(inputObj) + inputObj.offsetWidth + offset-170)+  'px';
			this.tooltipDiv.style.top = this.getTopPos(inputObj)-55 + 'px';
		}
